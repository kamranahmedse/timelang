...
import { parse, DateResult, SpanResult, DurationResult, FuzzyResult } from '../src';
...
describe('Edge Cases', () => {
  describe('Case insensitivity', () => {
    it('should parse "NEXT FRIDAY"', () => {
      const result = parse('NEXT FRIDAY', { referenceDate }) as DateResult;
    ...
    it('should parse "JANUARY 15TH FOR 2 WEEKS"', () => {
      const result = parse('JANUARY 15TH FOR 2 WEEKS', { referenceDate }) as SpanResult;
      expect(result.type).toBe('span');
      expect(result.start.toISOString()).toBe('2025-01-15T00:00:00.000Z');
      expect(result.end.toISOString()).toBe('2025-01-29T00:00:00.000Z');
      expect(result.duration).toBe(14 * MS_PER_DAY);
    });
  ...
  describe('Whitespace handling', () => {
    ...
    it('should parse "  next   friday   for   2   weeks  "', () => {
      const result = parse('  next   friday   for   2   weeks  ', { referenceDate }) as SpanResult;
      expect(result.type).toBe('span');
      expect(result.start.toISOString()).toBe('2025-01-17T00:00:00.000Z');
      expect(result.end.toISOString()).toBe('2025-01-31T00:00:00.000Z');
      expect(result.duration).toBe(14 * MS_PER_DAY);
    });
  });
  ...
  describe('Ambiguous inputs', () => {
    ...
    it('should handle "monday" (bare weekday)', () => {
      const result = parse('monday', { referenceDate }) as DateResult;
      expect(result.type).toBe('date');
      expect(result.date.toISOString()).toBe('2025-01-20T00:00:00.000Z');
    });
  ...
  describe('Year rollover', () => {
    ...
    it('should parse "january" in December as next January', () => {
      const result = parse('january 1', { referenceDate: decemberRef }) as DateResult;
      expect(result.type).toBe('date');
    ...
    it('should parse "last december" in January as previous December', () => {
      const result = parse('last december', { referenceDate: januaryRef }) as DateResult;
      expect(result.type).toBe('date');
    ...
    it('should parse "next january" in November as upcoming January', () => {
      const result = parse('next january', { referenceDate: novemberRef }) as DateResult;
      expect(result.type).toBe('date');
      expect(result.date.getUTCFullYear()).toBe(2026);
      expect(result.date.getUTCMonth()).toBe(0);
    });
    ...
    it('should parse "Q1" in Q4 as next Q1', () => {
      ...
      const result = parse('Q1', { referenceDate: q4Ref }) as FuzzyResult;
  ...
  describe('Leap years', () => {
    it('should parse "february 29th 2024" as valid (leap year)', () => {
      const result = parse('february 29th 2024', { referenceDate }) as DateResult;
      expect(result.type).toBe('date');
    ...
    it('should handle "february 29th" with referenceDate in 2024', () => {
      ...
      const result = parse('february 29th', { referenceDate: ref2024 }) as DateResult;
      expect(result.type).toBe('date');
      expect(result.date.getUTCMonth()).toBe(1);
      expect(result.date.getUTCDate()).toBe(29);
    });
  ...
  describe('Month boundaries', () => {
    it('should parse "january 31st" as valid', () => {
      const result = parse('january 31st', { referenceDate }) as DateResult;
      expect(result.type).toBe('date');
  ...
  describe('Duration edge cases', () => {
    it('should parse "1000 days"', () => {
      const result = parse('1000 days', { referenceDate }) as DurationResult;
      expect(result.type).toBe('duration');
      expect(result.duration).toBe(1000 * MS_PER_DAY);
    });

    it('should parse "100 years"', () => {
      const result = parse('100 years', { referenceDate }) as DurationResult;
      expect(result.type).toBe('duration');
      expect(result.duration).toBe(100 * 365 * MS_PER_DAY);
    });

    it('should parse "1 second"', () => {
      const result = parse('1 second', { referenceDate }) as DurationResult;
      expect(result.type).toBe('duration');
      expect(result.duration).toBe(1000);
    });

    it('should parse "0.001 days"', () => {
      const result = parse('0.001 days', { referenceDate }) as DurationResult;
      expect(result.type).toBe('duration');
      expect(result.duration).toBe(0.001 * MS_PER_DAY);
    });

    it('should parse "0 days"', () => {
      const result = parse('0 days', { referenceDate }) as DurationResult;
      expect(result.type).toBe('duration');
  ...
  describe('Time edge cases', () => {
    it('should treat "midnight", "12am", and "00:00" the same', () => {
      const r1 = parse('tomorrow at midnight', { referenceDate }) as DateResult;
      const r2 = parse('tomorrow at 12am', { referenceDate }) as DateResult;
      const r3 = parse('tomorrow at 00:00', { referenceDate }) as DateResult;
    ...
    it('should treat "noon", "12pm", and "12:00" the same', () => {
      const r1 = parse('tomorrow at noon', { referenceDate }) as DateResult;
      const r2 = parse('tomorrow at 12pm', { referenceDate }) as DateResult;
      const r3 = parse('tomorrow at 12:00', { referenceDate }) as DateResult;
  ...
  describe('Parenthetical edge cases', () => {
    ...
    it('should parse "Meeting (Team A) (Jan 15)" correctly', () => {
      const result = parse('Meeting (Team A) (Jan 15)', { referenceDate }) as DateResult;
      expect(result.type).toBe('date');
    ...
    it('should parse "Meeting (weekly) - Jan 15" correctly', () => {
      const result = parse('Meeting (weekly) - Jan 15', { referenceDate }) as DateResult;
      expect(result.type).toBe('date');
  ...
  describe('Return type validation', () => {
    ...
    it('should have valid Date object for date type', () => {
      const result = parse('tomorrow', { referenceDate }) as DateResult;
      expect(result.type).toBe('date');
    ...
    it('should have positive duration for duration type', () => {
      const result = parse('2 weeks', { referenceDate }) as DurationResult;
      expect(result.type).toBe('duration');
    ...
    it('should have start <= end for span type', () => {
      const result = parse('jan 5 to jan 20', { referenceDate }) as SpanResult;
      expect(result.type).toBe('span');
    ...
    it('should have title as string or null', () => {
      const r1 = parse('tomorrow', { referenceDate }) as DateResult;
      const r2 = parse('Meeting - tomorrow', { referenceDate }) as DateResult;
    ...
    it('should have approximate = true for fuzzy type', () => {
      const result = parse('Q1', { referenceDate }) as FuzzyResult;
      expect(result.type).toBe('fuzzy');
