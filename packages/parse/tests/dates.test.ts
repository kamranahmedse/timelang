...
import { DateResult, parse, parseDate, ParseOptions } from "../src";
...
function expectDate(
  input: string,
  expectedDate: Date,
  options: ParseOptions = { referenceDate },
) {
...
describe("Single Date Parsing", () => {
  ...
  describe("This/Coming/Previous weekdays", () => {
    it('should parse "this monday"', () => {
      expectDate("this monday", utc(2025, 1, 20)); // Next Monday (forward-looking)
    });
  ...
  describe("Month + Day + Year", () => {
    ...
    it('should parse "15/03/2025" (intl format)', () => {
      expectDate("15/03/2025", utc(2025, 3, 15), {
        referenceDate,
        dateFormat: "intl",
      });
    });
    ...
    it('should parse "03/15/2025" (us format)', () => {
      expectDate("03/15/2025", utc(2025, 3, 15), {
        referenceDate,
        dateFormat: "us",
      });
    });
