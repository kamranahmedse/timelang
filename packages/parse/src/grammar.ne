...
range -> date _ toConnector _ date {% d => makeRange(d[0], d[4]) %}
  ...
  return makeRange(d[0], makeDate({ month: end.month, day: end.day }));
       ...
       | fuzzy _ %dash _ fuzzy {% d => makeRange(d[0], d[4]) %}
...
span -> date _ forConnector _ duration {% d => makeSpan(d[0], d[4]) %}
      ...
      | duration _ afterConnector _ fuzzy _ forConnector _ duration {% d => makeSpanWithOffset(d[4], d[0], 'future', d[8]) %}
      | duration _ beforeConnector _ fuzzy _ forConnector _ duration {% d => makeSpanWithOffset(d[4], d[0], 'past', d[8]) %}
      | wordNumber _ unit _ afterConnector _ fuzzy _ forConnector _ duration {% d => makeSpanWithOffset(d[6], makeDuration(parseWordNumber(d[0]), d[2]), 'future', d[10]) %}
      | wordNumber _ unit _ beforeConnector _ fuzzy _ forConnector _ duration {% d => makeSpanWithOffset(d[6], makeDuration(parseWordNumber(d[0]), d[2]), 'past', d[10]) %}
...
relativeDate -> duration _ agoConnector {% d => makeRelativeDate('past', d[0]) %}
              ...
              | duration _ beforeConnector _ fuzzy {% d => ({ ...makeRelativeDate('past', d[0]), baseFuzzy: d[4] }) %}
              | duration _ afterConnector _ fuzzy {% d => ({ ...makeRelativeDate('future', d[0]), baseFuzzy: d[4] }) %}
              | wordNumber _ unit _ beforeConnector _ fuzzy {% d => ({ ...makeRelativeDate('past', makeDuration(parseWordNumber(d[0]), d[2])), baseFuzzy: d[6] }) %}
              | wordNumber _ unit _ afterConnector _ fuzzy {% d => ({ ...makeRelativeDate('future', makeDuration(parseWordNumber(d[0]), d[2])), baseFuzzy: d[6] }) %}
...
fuzzy -> quarter {% d => makeFuzzy({ period: 'quarter', quarter: parseQuarter(d[0].value) }) %}
       ...
       | endConnector _ month {% d => makeFuzzy({ period: 'month', month: d[2], modifier: 'end' }) %}
       | month _ endConnector {% d => makeFuzzy({ period: 'month', month: d[0], modifier: 'end' }) %}
       | midModifier _ ofConnector _ month {% d => makeFuzzy({ period: 'month', month: d[4], modifier: 'mid' }) %}
       | earlyModifier _ ofConnector _ month {% d => makeFuzzy({ period: 'month', month: d[4], modifier: 'early' }) %}
       | lateModifier _ ofConnector _ month {% d => makeFuzzy({ period: 'month', month: d[4], modifier: 'late' }) %}
       | wholeModifier _ month {% d => makeFuzzy({ period: 'month', month: d[2] }) %}
       | wholeModifier _ ofConnector _ month {% d => makeFuzzy({ period: 'month', month: d[4] }) %}
       ...
       | nextRelative _ quarterUnit {% d => makeFuzzy({ period: 'quarter', relative: 'next' }) %}
       | lastRelative _ quarterUnit {% d => makeFuzzy({ period: 'quarter', relative: 'last' }) %}
       | thisRelative _ quarterUnit {% d => makeFuzzy({ period: 'quarter', relative: 'this' }) %}
       | pastRelative _ quarterUnit {% d => makeFuzzy({ period: 'quarter', relative: 'past' }) %}
